...
| **Density** - Use an IF THEN expression with a check box<br> and a conditional expression to display the correct density<br> with the values given.	 | Medium	| [density.xlsm](files/density.xlsm)	           | [density_key.xlsm](files/density_key.xlsm)	|
| **Receipt** - Create an IF THEN expression that takes a<br> few conditions into consideration when generating<br> a receipt for a customer.	         | Hard	|                 [receipt.xlsm](files/receipt.xlsm)                 | [receipt_key.xlsm](files/receipt_key.xlsm)	|
