...
<p>As described in the <a href="../vb-variables/">Variables</a> chapter, you don't necessarily have to declare your variables before you use them. For example, if I want to start using a variable called <strong>myvar</strong>, I can simply write a line of code like this:</p>
<pre><code class="language-vb">myvar = Range("A4") * 2</code></pre>
	
<p>and my code will work just fine. When that line of code is executed, the VBA compiler looks at the myvar item and if it doesn't recognize it as a known variable or object, it assumes that you are declaring a new variable. That works fine until you write a line of code that accesses an existing variable but you mispell the variable name, thus accidentally creating a new variable with a default value of zero (or empty string, etc.). This leads to run-time errors that can be difficult to find and fix. This can all be avoided by getting into the habit of typing:</p>
<pre><code class="language-vb">Option Explicit</code></pre>
<p>at the top of your code everytime you start coding a new module or sheet.</p>
<h2>Indenting</h2>
...
Next mycol
Next myrow</code></pre>
<p>This code will generate precisely the same results as the first code when executed. However, it is incredibly difficult to follow the logic. Thus, it is much easier to make mistakes when do not indent. It is important to get into the habit of indenting your code. Just follow the example used in this Primer. We always indent lines of code that are in a logical block.</p>
<h2>Comments</h2>
...
<h2>Control Names</h2>
<p>As described in the <a href="../vb-controls/">Controls</a> chapter, using descriptive names for your controls is very important because it allows you to keep track of both the control's type and its function. You should always use descriptive control names and not use use the default names.</p>
<h2>Variable Names</h2>
<p>By the same logic, you should be careful when selecting your variable names. You should generally select variable names that accurately describe the intended use and contents of your variable. For example, suppose I am declaring an integer variable that will contain the number of students in a class. I could decare it as:</p>
<pre><code class="language-vb">Dim ns As Integer</code></pre>
<p>where ns stands for &quot;number of students&quot;. A better approach would be something like this:</p>
<pre><code class="language-vb">Dim numstudents As Integer</code></pre>
<p>Using descriptive variable names takes a little more effort but can make your code easier to follow and less prone to error.<br />
</p>
<?php 
require "../footer.php";
?>
